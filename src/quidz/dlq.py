...
__all__ = ["REASON_CODES", "ReasonCode", "RetryPolicy", "classify", "next_delay", "should_retry"]
...
def next_delay(policy: RetryPolicy, attempt: int, rng: random.Random) -> float:
    """Full jitter backoff. The rng is a seeded instance, never the global random module."""
    if attempt < 0:
